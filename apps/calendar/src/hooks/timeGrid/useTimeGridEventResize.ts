...
import { addMinutes, EVENT_MIN_DURATION, max, setTimeStrToDate } from '@src/time/datetime';
...
export function useTimeGridEventResize({
  ...
  useWhen(() => {
    ...
    if (shouldUpdate) {
      ...
      const minEndDate = addMinutes(resizingStartUIModel.getStarts(), EVENT_MIN_DURATION);
